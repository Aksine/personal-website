        ...
        stage('Clone Hugo App Repository') {
            ...
            container('helm') {
                stage('Install on Doha cluster') {
                        ...
                        sh "helm upgrade  personal-website bjw-s-charts/app-template  -f Helm.yml --version 3.7.3 --kube-context doh --set-string controllers.main.containers.main.image.tag=${env.BUILD_NUMBER}"
                ...
                stage('Install on Aus cluster') {
                        ...
                        sh ("helm repo add bjw-s-charts https://bjw-s-labs.github.io/helm-charts/")
                        sh "helm upgrade personal-website bjw-s-charts/app-template  -f Helm.yml --version 3.7.3 --kube-context aus --set-string controllers.main.containers.main.image.tag=${env.BUILD_NUMBER}"
